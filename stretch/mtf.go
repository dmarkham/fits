...
import "github.com/dmarkham/fits/stats"
...
func AutostretchParams(pixels []float32, shadowsClip, targetBG float32) (shadows, midtones, highlights float32) {
	good := stats.FilterNonZero(pixels)
	med := float32(stats.Median(good))
	mad := float32(stats.MAD(good))
